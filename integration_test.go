...
type integrationAudit struct {
	ID      string `bson:"_id"`
	Message string `bson:"message" db:"index"`
}
...
func TestTypedSDK(t *testing.T) {
	...
	fields := sdkmongo.Map().Set("name", "Alice Updated")
	updated, err = db.Update[integrationUser](ctx, "001", fields)
	require.NoError(t, err)
	require.Equal(t, "Alice Updated", updated.Name)

	updated, err = db.Increment[integrationUser](ctx, "001", sdkmongo.Map().Set("score", 5))
	require.NoError(t, err)
	require.Equal(t, int64(15), updated.Score)

	exists, err := db.Find[integrationUser](ctx, sdkmongo.Map().Set("email", "alice@example.com")).Exists()
	require.NoError(t, err)
	require.True(t, exists)

	activeFilter := sdkmongo.Map().Set("active", true)
	...
	first, err := db.Find[integrationUser](ctx).Sort(sdkmongo.Map().Set("age", -1)).First()
	require.NoError(t, err)
	require.Equal(t, "003", first.ID)

	next, err := db.Find[integrationUser](ctx).After("001").Limit(2).All()
	require.NoError(t, err)
	require.Len(t, next, 2)
	require.Equal(t, "002", next[0].ID)
	require.Equal(t, "003", next[1].ID)
	remaining, err := db.Find[integrationUser](ctx).After("001").Count()
	require.NoError(t, err)
	require.Equal(t, int64(2), remaining)

	previous, err := db.Find[integrationUser](ctx).After("003").Desc().Limit(2).All()
	require.NoError(t, err)
	...
	err = db.Find[integrationUser](ctx).Batch(2).Each(func(user *integrationUser) (bool, error) {
	...
	updateFilter := sdkmongo.Map().Set("active", true)
	updateFields := sdkmongo.Map().Set("active", false)
	...
	deleted, err := db.DeleteMany[integrationUser](ctx, sdkmongo.Map().Set("active", false))
	require.NoError(t, err)
